...
- 使用始皇的：[ja-netfilter](https://gitee.com/ja-netfilter/ja-netfilter) 激活，相关文件已在本仓库中，参考如下步骤激活：
    ...
    3. 打开并进入 JetBrains IDE，点击顶部菜单：`Help --> Edit Custom VM Options...`（参考 [issue 13](https://github.com/zpj80231/idea-set/issues/13#issuecomment-3526721400)）
       ...
       # 4.注意：-javaagent 最好不要重复，防止造成 invalid code 和重合互锁。参考 [issue 20](https://github.com/zpj80231/idea-set/issues/20)
...
- ~~付费插件激活（已下架自用）：~~
    - ~~认证方式（使用佬友提供的 LINUX.DO 认证提高使用门槛，认证需自行解决）：配合猴油脚本，手动添加脚本到猴油中，打开 [https://plugins.jetbrains.com](https://plugins.jetbrains.com/) 搜索你想要激活的插件，点击 `Generate Code` 按钮就能得到该插件的激活码，粘入到插件的 `Activation Code` 中即可~~
    - ~~离线方式（影响太大，已下架自用）~~
